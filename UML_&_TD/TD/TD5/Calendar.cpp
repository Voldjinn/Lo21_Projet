...
/* pas bien optimisé
void TacheManager::Iterator::next(){
    if(isDone())
        throw CalendarException("on est deja a la fin cassos");
    ++pt;
    --nbRestant;
}
Tache& TacheManager::Iterator::current() const{
    if(isDone())
        throw CalendarException("fin atteinte");
    return **pt;
}*/
